...
class EvalMaxSAT {
    ...
    ///
        ...
        struct LitCard {
            std::shared_ptr<CardIncremental_Lazy<EvalMaxSAT> > card;
            int atMost;
            t_weight initialWeight;

            LitCard(std::shared_ptr<CardIncremental_Lazy<EvalMaxSAT>> card, int atMost, t_weight initialWeight) : card(card), atMost(atMost), initialWeight(initialWeight) {
                assert( card != nullptr );
                assert(atMost > 0);
...
public:
    using ExternalPropagator = CaDiCaL::ExternalPropagator;
    ...
    public:
    EvalMaxSAT() : solver(new Solver_cadical) {
        ...
        for(;;) {
            ...
            if(resultLastSolve) { // SAT
                    ...
                    if(resultLastSolve == 0) { // UNSAT
                        ...
                        if(_litToRelax.size()) {
                            ...
                            while(_cardToAdd.size()) {
                                std::shared_ptr<CardIncremental_Lazy<EvalMaxSAT>> card = std::make_shared<CardIncremental_Lazy<EvalMaxSAT>>(this, std::get<0>(_cardToAdd.front()), 1);
                ...
                for(auto c: _cardToAdd) {
                    std::shared_ptr<CardIncremental_Lazy<EvalMaxSAT>> card = std::make_shared<CardIncremental_Lazy<EvalMaxSAT>>(this, std::get<0>(c), 1);
                    int newAssumForCard = card->atMost(1);
                    if(newAssumForCard != 0) {
                        assert( _poids[newAssumForCard] == 0 );
